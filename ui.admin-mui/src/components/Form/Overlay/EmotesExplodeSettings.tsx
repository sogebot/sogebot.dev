import { EmotesExplode } from '@entity/overlay';
...
type Props = {
  model:    EmotesExplode;
  onUpdate: (value: EmotesExplode) => void;
};
...
export const EmotesExplodeSettings: React.FC<Props> = ({ model, onUpdate }) => {
  ...
  const test = () => {
    axios.post('/api/core/emotes?_action=testExplosion', undefined, {
      headers: {
    ...
    <Stack spacing={0.5}>
      ...
      <TextField
        ...
        onChange={(ev) => {
          if (!isNaN(Number(ev.currentTarget.value))) {
            onUpdate({
              ...model, numOfEmotes: Number(ev.currentTarget.value),
            });
          }
        }}
      />
