...
import {
  ...
  ListItemIcon,
  ListItemText, ListSubheader, Stack, TextField, Typography,
} from '@mui/material';
...
import { CodeTwoTone, ContentPasteTwoTone, DriveFileRenameOutlineTwoTone, LinkTwoTone } from '@mui/icons-material';
                ...
                <li key={`section-source-files`}>
                  <ul>
                    <ListSubheader sx={{ display: 'flex', justifyContent: 'space-between', py: 0.5, alignItems: 'center' }}>
                      <Button fullWidth color={fileType === 'code' ? 'primary' : 'light'} onClick={() => setFileType('code')}>Source files</Button>
                      <Typography sx={{ px: 2 }} variant='button'>/</Typography>
                      <Button fullWidth color={fileType === 'overlay' ? 'primary' : 'light'} onClick={() => setFileType('overlay')}>Overlay files</Button>
                    </ListSubheader>
