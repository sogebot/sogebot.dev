...
export const DashboardStatsGeneralTips: React.FC = () => {
  ...
  const average = useMemo(() => {
    return averageStats.currentTips;
  }, [averageStats]);

  useDidMount(() => {
    getSocket('/').on('panel::stats', async (data: Record<string, any>) => {
      setValue(data.currentTips);
      setLoading(false);
    });
  });
  ...
  return (
    <Grid item xs={6} sm={4} md={4} lg={2}>
      <Paper sx={{
        ...
        <Typography color={theme.palette.grey[400]} variant='caption' sx={{ pt: 2, pa: 1 }}>{ capitalize(translate('tips')) }</Typography>
      </Paper>
    </Grid>
  );
};