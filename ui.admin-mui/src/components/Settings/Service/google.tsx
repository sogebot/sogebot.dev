...
const PageSettingsModulesServiceGoogle: React.FC<{
  ...
  const refreshStreams = useCallback(async () => {
    await Promise.all([
      new Promise<void>(resolve => {
        axios.get(`${JSON.parse(sessionStorage.server)}/api/services/google/streams`, { headers: { authorization: `Bearer ${getAccessToken()}` } })
          .then(({ data }) => {
            console.log({ data });
  ...
  const handleSave = useCallback(async () => {
    ...
    for (const key of privateKeys) {
      if (key.privateKey) {
        ...
        await axios.post(`${JSON.parse(sessionStorage.server)}/api/services/google/privatekeys`, {
          id:          key.id,
    ...
    for (const key of privateKeysCache) {
      if (!privateKeys.find(o => o.id === key.id)) {
        axios.delete(`${JSON.parse(sessionStorage.server)}/api/services/google/privatekeys/${key.id}`, { headers: { authorization: `Bearer ${getAccessToken()}` } });
      }
    }
    setSaving2(false);
  ...
  const authorize = useCallback(() => {
    const popup = window.open('/credentials/google', 'popup', 'popup=true,width=500,height=500,toolbar=no,location=no,status=no,menubar=no');
