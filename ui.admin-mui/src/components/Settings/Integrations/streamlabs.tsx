...
const PageSettingsModulesIntegrationsStreamlabs: React.FC<{
  ...
  const authorize = useCallback(() => {
    const popup = window.open((process.env.PUBLIC_URL !== '/' ? window.location.origin + '/' : process.env.PUBLIC_URL) + 'credentials/streamlabs', 'popup', 'popup=true,width=500,height=500,toolbar=no,location=no,status=no,menubar=no');
