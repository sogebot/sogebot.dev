...
async function test() {
  await new Promise((resolve, reject) => {
    try {
      exec('npx typeorm migration:run -d build/data-source.js', {
        env: {
          ...process.env,
  ...
  try {
    await new Promise((resolve, reject) => {
      ...
      const out2 = spawn(process.platform === 'win32' ? 'npx.cmd' : 'npx', 'typeorm migration:generate  -d build/data-source.js -n generatedMigration'.split(' '), {
        env: {
          ...process.env,
          'TYPEORM_ENTITIES':   'build/entity/*.js',
          'TYPEORM_MIGRATIONS': `build/migration/*.js`,
